...
# -- Función llenar factibilidades (MODIFICADA)
...
# --- Nuevos campos de entrada para Factibilidades ---
# Usamos un if para mostrar los campos adicionales solo si el formato es "Factibilidades"
...
if formato_seleccionado == "Cartera":
    ...
    # Lógica de subida de archivos para otros formatos
    ...
    # Se ajusta la lógica de la vista previa para factibilidades para que use dos columnas
    if ejecutor or direccion or fecha_visita or operador or uploaded_files:
        st.subheader("Vista Previa de los Datos:")
        # Se añaden los nuevos campos a la vista previa
        if formato_seleccionado == "Factibilidades":
            st.write(f"**Ejecutor:** {ejecutor}")
            st.write(f"**Teléfono Ejecutor:** {telefono_ejecutor}")
            st.write(f"**Encargado:** {encargado}")
            st.write(f"**Teléfono Encargado:** {telefono_encargado}")
            st.write(f"**Atiende en Sitio:** {atiende_en_sitio}")
            st.write(f"**Teléfono Atiende en Sitio:** {telefono_atiende_sitio}")
            st.write(f"**Nombre del sitio:** {cliente}")
        else:
            if ejecutor:
                st.write(f"**Ejecutor:** {ejecutor}")
            if direccion:
        ...
        if uploaded_files and formato_seleccionado != "Cartera":
            ...
            num_filas_preview = (len(uploaded_files) + 1) // 2
...
if st.button("Enviar a Drive"):
    if ejecutor and direccion and fecha_visita and uploaded_files:
        if formato_seleccionado == "Factibilidades" and (not cliente or not telefono_ejecutor or not encargado or not telefono_encargado or not atiende_en_sitio or not telefono_atiende_sitio):
            ...
            try:
                # --- Preparación de los datos y plantilla ---
                ...
                for i, archivo_subido in enumerate(uploaded_files):
                    ...
                    # --- Lógica de descripción (AJUSTADA PARA FACTIBILIDADES) ---
                    if (i + 1) % 2 != 0:
